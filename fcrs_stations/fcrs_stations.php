    ...
    switch ($case)
    {
        ...
        case "live.dbpedia":
			...
			SELECT DISTINCT ?dbpediaUri ?wikiPageRedirects ?sameAs ?redirectSameAs WHERE {
				...
				FILTER (!BOUND(?wikiPageDisambiguates)).
        		FILTER(lang(?label)="en").
				FILTER(STRSTARTS(STR(?dbpediaUri),"http://dbpedia.org/resource") || STRSTARTS(STR(?dbpediaUri),"http://live.dbpedia.org/resource")).
        		FILTER('.$regex.').
				'.$optionals.'
			} LIMIT 20
        	';
...
// Inpout: Array of Payment Agents
function psgrDdpediaMapping($agents, $outfile1, $outfile2 ,$outfile3, $dbpedia, $write_to_file, $from, $to) 
{
  global $NL;
  ...
  $agents = array_slice($agents, $from, $to);
  ...
  foreach ($agents as $rec) {
  	...
  	if (!empty($rec[0]) && !empty($rec[1]) /*&& !empty($rec[2]) && !empty($rec[3])*/) {
	    ...
	    $agent_domains = array();
	    $dout_agent_domains = "";
	    if (strpos($agent_name,'ΚΟΙΝΟΠΡΑΞΙΑ')!== false) $agent_domains['ΚΟΙΝΟΠΡΑΞΙΑ'] = 'ΚΟΙΝΟΠΡΑΞΙΑ';
	    ...
	    //
	    // Print payment agent Info
	    $dout_agent_name = '<a href="'.$agent_uri.'" target="_blank">'.$agent_name.'</a>';
	    ...
	    if ($dbpedia == "el") {
	    	if ($cnt_num_el == "1") $cnt1++;
	    	if ($cnt_num_el == "2") $cnt2++;
	    	if ($cnt_num_el == "3") $cnt3++;
	    }	 
  		if ($dbpedia == "en") {
	    	if ($cnt_num_en == "1") $cnt1++;
	    	if ($cnt_num_en == "2") $cnt2++;
	    	if ($cnt_num_en == "3") $cnt3++;
  		}
	    if ($dbpedia == "live") {
	    	if ($cnt_num_live == "1") $cnt1++;
	    	if ($cnt_num_live == "2") $cnt2++;
	    	if ($cnt_num_live == "3") $cnt3++;
	    }
	    if ($dbpedia == "all") {
		    if ($cnt_num_el == "2" || $cnt_num_en == "2" || $cnt_num_live == "2") $cnt2++;
		    else if ($cnt_num_el == "1" && $cnt_num_en == "1" && $cnt_num_live == "1") $cnt1++;
		    else $cnt3++;
	    }
...
function handleResults($agent_uri, $results, $agent_domains, $write_to_file, $outfile, &$cnt_num) {
	...
	elseif ((!empty($agent_domains['ΚΟΙΝΟΠΡΑΞΙΑ']) && $cnt<=15) || ($cnt<=5)) {
